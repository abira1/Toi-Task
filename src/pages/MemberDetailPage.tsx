...
import { Trophy, Flame, Target, CheckCircle2, Clock, ArrowLeft } from 'lucide-react';
...
export function MemberDetailPage({ member, tasks, onBack }: MemberDetailPageProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <button
        onClick={onBack}
        className="flex items-center gap-2 mb-4 sm:mb-6 text-[var(--black)] font-bold hover:text-[var(--teal)] transition-colors group"
      ...
      <div className="hidden sm:grid sm:grid-cols-3 gap-4 mb-8">
        {profileStats.map((stat, i) => {
          ...
          return (
            ...
            >
              <div className={`w-12 h-12 ${stat.color} rounded-xl flex items-center justify-center text-white border-2 border-[var(--black)] shadow-[4px_4px_0px_0px_var(--black)]`}>
        ...
        <div>
          ...
          <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
            <button
              ...
              className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-xl font-bold text-xs sm:text-sm transition-all whitespace-nowrap ${
                activeTab === 'pending'
                  ? 'bg-[var(--mustard)] text-[var(--black)] border-2 border-[var(--black)] shadow-[2px_2px_0px_0px_var(--black)] sm:shadow-[3px_3px_0px_0px_var(--black)]'
            ...
            <button
              ...
              className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-xl font-bold text-xs sm:text-sm transition-all whitespace-nowrap ${
                activeTab === 'done'
                  ? 'bg-[var(--teal)] text-white border-2 border-[var(--black)] shadow-[2px_2px_0px_0px_var(--black)] sm:shadow-[3px_3px_0px_0px_var(--black)]'
                  : 'bg-gray-100 text-gray-500 active:bg-gray-200'
              }`}
            >
          ...
          <div className="space-y-2 sm:space-y-3">
            {displayTasks.length > 0 ? (
              displayTasks.map((task) => (
                <div
                  key={task.id}
                  className={`flex items-center gap-3 p-3 sm:p-4 bg-white border-2 border-[var(--black)] rounded-xl ${
                    task.completed ? 'opacity-75' : ''
                  }`}
                >
                  <div
                    className={`w-6 h-6 rounded-full border-2 border-[var(--black)] flex items-center justify-center flex-shrink-0 ${
                      task.completed ? 'bg-[var(--teal)] text-white' : 'bg-white'
                    }`}
                  >
